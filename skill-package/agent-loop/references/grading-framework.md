...
**When:** After all subagents complete their deliverables, BEFORE grading begins.

**Why:** Parallel agents working in isolation will independently estimate shared facts — market sizes, growth rates, competitor metrics, timelines, cost figures. If Agent A says TAM is $50B and Agent C assumes $30B, the compiled draft contains a silent contradiction that grading may or may not catch. This protocol catches it systematically.

### How It Works

1. **Extract claims.** The orchestrator reads each deliverable and extracts every quantitative claim and key qualitative assertion that appears in more than one agent's work. Build a table:

```
| Claim | Agent A Value | Agent B Value | Agent C Value | Status |
|-------|--------------|--------------|--------------|--------|
| Total addressable market | $50B (T1) | — | $30B (T3) | CONFLICT |
| Annual growth rate | 12% CAGR (T1) | 8% CAGR (T2) | — | CONFLICT |
| Primary competitor market share | 35% (T1) | 35% (T2) | — | ALIGNED |
| Recommended timeline | 90 days (T3) | — | 120 days (T4) | CONFLICT |
```

2. **Resolve conflicts.** For each CONFLICT row:
   - **Check source materials first.** If the user provided data that answers the question, use that figure and correct all deliverables.
   - **Check citations.** If one agent cites a specific, verifiable source and the other doesn't, prefer the cited figure.
   - **Check the roundtable record.** If the roundtable agreed on a figure during convergence, use that.
   - **Flag to the user.** If none of the above resolves it, present the conflict and ask the user to set the canonical value.

3. **Update deliverables.** Before compiling the draft, correct all deliverables to use the resolved values. Log every correction in `grading.md` under a `## Shared Assumptions Resolution` header.

4. **Feed into grading.** The Cross-Agent Consistency criterion category (below) verifies that the resolution was applied correctly.

### Domain-Neutral Examples

- **Product launch:** Agent A's pricing analysis assumes 10,000 units in Year 1; Agent B's supply chain plan assumes 25,000 units. The launch plan is incoherent until these align.
- **Research synthesis:** Agent A cites a study showing 15% efficacy; Agent C cites the same study at 22%. One misread the source.
- **Creative campaign:** Agent A's brand positioning emphasizes premium exclusivity; Agent B's media plan targets mass-market channels. The strategy contradicts itself.
- **Operational plan:** Agent A budgets $500K for the migration; Agent C's timeline assumes a $200K budget. The plan is unexecutable.
